@extends('layouts.app', ['title' => 'Import Server Storage — PLN Financial'])
    ...
    <div class="flex-1 min-w-0">
        <main class="p-6 lg:p-10 space-y-5 w-full">
            <nav class="flex items-center gap-1.5 text-sm">
                ...
                <span class="font-semibold text-gray-700">Import Server Storage</span>
            </nav>

            <h1 class="text-2xl font-bold text-pln-800">Import Data Server Storage dari Excel/CSV</h1>
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
                <div class="flex items-start gap-4 p-8 pb-6">
                    ...
                    <div>
                        <h2 class="text-base font-bold text-pln-800">Upload File Excel / CSV Server Storage</h2>
                        <p class="text-sm text-gray-400">Unggah file data secara massal berdasarkan template standar Server Storage.</p>
                    </div>
                </div>

                <div class="border-t border-gray-100 p-8 space-y-5">
                    <form action="{{ route('manage-server-storage.import.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-6">
                        ...
                        <div class="flex items-center justify-end gap-3 border-t border-gray-200 pt-4">
                            <a href="{{ route('manage-server-storage') }}" class="px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-600 hover:bg-gray-50">Batal</a>
