@extends('layouts.app', ['title' => (($asset ?? false) ? 'Edit Server Storage' : 'Tambah Server Storage') . ' — PLN Financial'])

@section('content')
<main class="p-6 lg:p-10 space-y-5 w-full">
    <nav class="flex items-center gap-1.5 text-sm">
        <a href="{{ route('manage-server-storage') }}" class="text-gray-400 hover:text-[#004A54]">Server Storage</a>
        ...
        <span class="font-semibold text-gray-700">{{ ($asset ?? false) ? 'Edit Storage' : 'Tambah Storage' }}</span>
    </nav>

    <div>
        <h1 class="text-2xl font-bold text-pln-800">{{ ($asset ?? false) ? 'Edit Aset Server Storage' : 'Tambah Aset Server Storage Baru' }}</h1>
    </div>
    ...
    <form method="POST" action="{{ ($asset ?? false) ? route('manage-server-storage.update', $asset->id) : route('manage-server-storage.store') }}" class="bg-white rounded-xl border border-gray-200 shadow-sm">
        @csrf
        ...
        <div class="p-8 space-y-6">
            ...
            {{-- ATRIBUT SPESIFIK SERVER STORAGE --}}
            <h3 class="text-xs font-bold uppercase tracking-wider text-[#004A54] mt-8 mb-4 pb-2 border-b">Atribut Spesifik Server Storage</h3>
            
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div>
                    <label class="text-xs font-bold text-gray-700">Hostname</label>
                    <input type="text" name="hostname" value="{{ old('hostname', $asset->hostname ?? '') }}" placeholder="Contoh: SRV-STRG-01" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">
                </div>
                <div>
                ...
                <div>
                    ...
                    <input type="text" name="model" value="{{ old('model', $asset->model ?? '') }}" required placeholder="Contoh: PowerVault ME4024" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mt-4">
                <div>
                    <label class="text-xs font-bold text-gray-700">CPU / Controller</label>
                    <input type="text" name="cpu_controller" value="{{ old('cpu_controller', $asset->cpu_controller ?? '') }}" placeholder="Contoh: Dual Controller" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-700">RAM</label>
                    <input type="text" name="ram" value="{{ old('ram', $asset->ram ?? '') }}" placeholder="Contoh: 16GB Cache" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-700">Disk Array</label>
                    <input type="text" name="disk_array" value="{{ old('disk_array', $asset->disk_array ?? '') }}" placeholder="Contoh: 24x 1.2TB SAS, RAID 6" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-700">Kapasitas Total Storage</label>
                    <input type="text" name="kapasitas_total_storage" value="{{ old('kapasitas_total_storage', $asset->kapasitas_total_storage ?? '') }}" placeholder="Contoh: 20TB" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mt-4">
                ...
                <div>
                    ...
                    <input type="text" name="versi_firmware" value="{{ old('versi_firmware', $asset->versi_firmware ?? '') }}" placeholder="Contoh: v7.2.4" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-700">App Standar Kamsiber</label>
                    <select name="app_standar_kamsiber" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">
                        <option value="">— Pilih —</option>
                        @foreach(['tidak ada', 'AV', 'UEM', 'EDR', 'AV-UEM-EDR', 'AV-UEM', 'AV-EDR', 'UEM-EDR'] as $kamsiber)
                            <option value="{{ $kamsiber }}" @selected(old('app_standar_kamsiber', $asset->app_standar_kamsiber ?? '') == $kamsiber)>{{ $kamsiber }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="mt-4">
                ...
                <textarea name="aplikasi" rows="2" placeholder="Daftar aplikasi yang menggunakan storage ini" class="mt-1.5 w-full rounded-md border border-gray-300 py-2 px-3 text-sm">{{ old('aplikasi', $asset->aplikasi ?? '') }}</textarea>
            </div>
        </div>

        <div class="flex justify-end gap-3 px-8 py-5 border-t bg-gray-50 rounded-b-xl">
            <a href="{{ route('manage-server-storage') }}" class="border border-gray-300 text-gray-700 px-6 py-2.5 rounded-md text-sm hover:bg-gray-100">Batal</a>
            <button type="submit" class="bg-[#004A54] text-white px-6 py-2.5 rounded-md text-sm font-medium hover:bg-[#00363d]">
                {{ ($asset ?? false) ? 'Simpan Perubahan' : 'Simpan Storage' }}
            </button>
        </div>
    </form>
</main>
