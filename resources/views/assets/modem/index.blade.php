@extends('layouts.app', ['title' => 'Manage Modem — PLN Financial'])
    ...
    <div class="flex-1 min-w-0">
        ...
        <div class="px-6 space-y-4">
            <nav class="flex items-center gap-1.5 text-sm pt-4">
                <a href="{{ route('dashboard') }}" class="text-gray-400 dark:text-gray-500 hover:text-[#004A54]">Dashboard</a>
                <svg class="w-3.5 h-3.5 text-gray-300" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5" />
                </svg>
                <span class="font-semibold text-gray-700 dark:text-gray-200">Manage Asset: Modem</span>
            </nav>

            <div class="flex items-center justify-between flex-wrap gap-3">
                <div>
                    <p class="text-[10px] text-gray-400 uppercase tracking-wider font-bold">MANAGE ASSET</p>
                    <h1 class="text-xl font-bold text-pln-800 dark:text-white tracking-wide">Daftar Asset Modem</h1>
                </div>
                <div class="flex items-center gap-2">
                    <a href="{{ route('manage-modem.import.form') }}" class="inline-flex items-center gap-2 border border-gray-300 text-gray-700 px-4 py-2 rounded-md text-sm font-medium hover:bg-gray-50">
                    ...
                    <a href="{{ route('manage-modem.create') }}" class="inline-flex items-center gap-2 bg-[#004A54] text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-[#00363d]">
        ...
        <div class="mx-6 mt-4 bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
            <form method="GET" action="{{ route('manage-modem') }}" class="flex flex-row flex-wrap items-center gap-4">
                <div class="flex-1 min-w-[250px]">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari ID, merk, model, SN, IP, lokasi..." class="w-full rounded-lg border border-gray-300 py-2.5 px-4 text-sm focus:border-[#004A54] focus:outline-none">
                </div>
                <div>
                ...
                <div>
                    <a href="{{ route('manage-modem') }}" class="inline-block border-2 border-[#004A54] text-[#004A54] px-5 py-2 rounded-lg text-sm font-bold hover:bg-cyan-50">Reset</a>
                </div>
            </form>
        </div>

        {{-- TABLE (SESUAI TEMPLATE EXCEL MODEM) --}}
        <div class="mx-6 mt-4 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-10">
                ...
                <table class="w-full text-left border-collapse whitespace-nowrap">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-200 text-xs font-bold uppercase tracking-wider text-gray-500">
                            ...
                            <th class="px-5 py-4">Tipe Koneksi</th>
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm">
                        @forelse(($modems ?? []) as $modem)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                <td class="px-5 py-4 font-semibold text-[#004A54]">{{ $modem->id_aset ?? '-' }}</td>
                                <td class="px-5 py-4">
                                    <div class="font-medium text-gray-800 text-xs">{{ $modem->merk ?? '-' }}</div>
                                    <div class="text-gray-400 text-[10px] mt-0.5">{{ $modem->model ?? '-' }}</div>
                                </td>
                                <td class="px-5 py-4 font-mono text-xs text-gray-700">{{ $modem->serial_number ?? '-' }}</td>
                                <td class="px-5 py-4 font-mono text-xs text-gray-700">
                                    <div>{{ $modem->ip_address ?? '-' }}</div>
                                    <div class="text-[10px] text-gray-400">{{ $modem->mac_address ?? '-' }}</div>
                                </td>
                                <td class="px-5 py-4 text-xs font-medium text-gray-600">{{ $modem->tipe_koneksi ?? '-' }}</td>
                                <td class="px-5 py-4 text-xs text-gray-700">
                                    <div>{{ $modem->lokasi_aset_saat_ini ?? '-' }}</div>
                                    <div class="text-[10px] text-gray-400">{{ $modem->keterangan_lokasi_aset ?? '-' }}</div>
                                </td>
                                <td class="px-5 py-4">
                                    <div class="flex items-center gap-1.5">
                                        <span class="inline-flex px-2 py-0.5 rounded-full text-[11px] font-medium bg-green-100 text-green-700">
                                            {{ ucfirst($modem->status_kondisi ?? 'baik') }}
                                        </span>
                                        <span class="inline-flex px-2 py-0.5 rounded-full text-[11px] font-medium bg-blue-100 text-blue-700">
                                            {{ ucfirst($modem->status_operasional ?? 'aktif') }}
                                        </span>
                                    </div>
                                </td>
                                <td class="px-5 py-4 text-right sticky right-0 bg-white shadow-l">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('manage-modem.edit', $modem->id) }}" title="Edit" class="w-9 h-9 flex items-center justify-center rounded-md border border-gray-200 text-gray-500 hover:text-[#004A54] hover:border-[#004A54]">
                                        ...
                                        <form action="{{ route('manage-modem.destroy', $modem->id) }}" method="POST" onsubmit="return confirm('Hapus modem {{ $modem->id_aset }}?')">
                                            @csrf @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-5 py-16 text-center text-gray-400">Belum ada data Modem.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            @if(isset($modems) && method_exists($modems, 'total'))
                <div class="px-5 py-4 border-t border-gray-100 text-sm text-gray-500 flex items-center justify-between">
                    <div>Menampilkan {{ $modems->firstItem() ?? 0 }} - {{ $modems->lastItem() ?? 0 }} dari {{ $modems->total() }} data</div>
                    <div>{{ $modems->onEachSide(1)->links() }}</div>
                </div>
            @endif
        </div>
    </div>
