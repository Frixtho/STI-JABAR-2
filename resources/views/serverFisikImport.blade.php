@extends('layouts.app', ['title' => 'Import Server Fisik — PLN Financial'])
    ...
    <div class="flex-1 min-w-0">
        <main class="p-6 lg:p-10 space-y-5 w-full">
            <nav class="flex items-center gap-1.5 text-sm">
                ...
                <span class="font-semibold text-gray-700">Import Server Fisik</span>
            </nav>

            <h1 class="text-2xl font-bold text-pln-800">Import Data Server Fisik dari Excel/CSV</h1>
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
                <div class="flex items-start gap-4 p-8 pb-6">
                    ...
                    <div>
                        <h2 class="text-base font-bold text-pln-800">Upload File Excel / CSV Server Fisik</h2>
                        <p class="text-sm text-gray-400">Unggah file data secara massal berdasarkan template Server Fisik (mendukung hingga kolom Virtual Server).</p>
                    </div>
                </div>

                <div class="border-t border-gray-100 p-8 space-y-5">
                    <form action="{{ route('manage-server-fisik.import.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-6">
                            <label class="block text-sm font-semibold mb-1 text-gray-700">Pilih File CSV</label>
                            <input type="file" name="files[]" accept=".csv" class="w-full border border-gray-300 rounded-lg p-2 text-sm bg-white file:mr-4 file:py-1.5 file:px-3 file:rounded-md file:bg-[#004A54] file:text-white cursor-pointer" multiple required>
                        </div>
                        <div class="flex items-center justify-end gap-3 border-t border-gray-200 pt-4">
                            <a href="{{ route('manage-server-fisik') }}" class="px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-600 hover:bg-gray-50">Batal</a>
