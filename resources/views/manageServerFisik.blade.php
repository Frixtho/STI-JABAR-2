@extends('layouts.app', ['title' => 'Manage Server Fisik — PLN Financial'])
    ...
    <div class="flex-1 min-w-0">
        ...
        <header class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700 px-6 py-4 flex items-center justify-between gap-4">
            <div class="relative w-full max-w-sm">
                ...
                <input type="text" placeholder="Cari data..."
                    class="w-full rounded-md border border-gray-200 dark:border-gray-600 bg-gray-50 py-2 pl-10 pr-3 text-sm focus:border-pln-700 focus:outline-none focus:ring-2 focus:ring-pln-700/20">
            </div>
        ...
        <div class="px-6 space-y-4">
            <nav class="flex items-center gap-1.5 text-sm pt-4">
                ...
                <span class="font-semibold text-gray-700">Manage Asset: Server Fisik (Virtualisasi)</span>
            </nav>

            <div class="flex items-center justify-between flex-wrap gap-3">
                <div>
                    <p class="text-[10px] text-gray-400 uppercase tracking-wider font-bold">MANAGE ASSET</p>
                    <h1 class="text-xl font-bold text-pln-800 tracking-wide">Daftar Server Fisik (Host Virtualisasi)</h1>
                </div>
                <div class="flex items-center gap-2">
                    <a href="{{ route('manage-server-fisik.import.form') }}" class="inline-flex items-center gap-2 border border-gray-300 text-gray-700 px-4 py-2 rounded-md text-sm font-medium hover:bg-gray-50">
                    ...
                    <a href="{{ route('manage-server-fisik.create') }}" class="inline-flex items-center gap-2 bg-[#004A54] text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-[#00363d]">
        ...
        <div class="mx-6 mt-4 bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
            <form method="GET" action="{{ route('manage-server-fisik') }}" class="flex flex-row flex-wrap items-center gap-4">
                ...
                <div>
                    <a href="{{ route('manage-server-fisik') }}" class="inline-block border-2 border-[#004A54] text-[#004A54] px-5 py-2 rounded-lg text-sm font-bold hover:bg-cyan-50">Reset</a>
                </div>
            </form>
        </div>
                ...
                <table class="w-full text-left border-collapse whitespace-nowrap">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-200 text-xs font-bold uppercase tracking-wider text-gray-500">
                            ...
                            <th class="px-5 py-4">VMs Info</th>
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm">
                        @forelse(($servers ?? []) as $server)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="px-5 py-4 text-xs text-gray-700">
                                    <div>CPU: <span class="font-medium">{{ $server->cpu ?? '-' }}</span></div>
                                    <div class="text-[10px] text-gray-500 mt-0.5">RAM: {{ $server->ram ?? '-' }} | Strg: {{ $server->kapasitas_total_storage ?? '-' }}</div>
                                </td>
                                <td class="px-5 py-4 text-xs text-gray-700">
                                    <div class="truncate max-w-[150px]" title="{{ $server->hostname_virtual }}">{{ $server->hostname_virtual ?: '-' }}</div>
                                    <div class="text-[10px] text-gray-500 mt-0.5">vCPU: {{ $server->jumlah_vcpu ?: '-' }} | App: <span class="truncate">{{ $server->aplikasi_virtual ?: '-' }}</span></div>
                                </td>
                                <td class="px-5 py-4 text-xs text-gray-700">
                                ...
                                <td class="px-5 py-4 text-right sticky right-0 bg-white shadow-l">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('manage-server-fisik.edit', $server->id) }}" title="Edit" class="w-9 h-9 flex items-center justify-center rounded-md border border-gray-200 text-gray-500 hover:text-[#004A54] hover:border-[#004A54]">
                                        ...
                                        <form action="{{ route('manage-server-fisik.destroy', $server->id) }}" method="POST" onsubmit="return confirm('Hapus server {{ $server->id_aset }}?')">
                                            @csrf @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-5 py-16 text-center text-gray-400">Belum ada data Server Fisik.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
