@extends('layouts.app', ['title' => 'Manage UPS — PLN Financial'])
    ...
    <div class="flex-1 min-w-0">
        ...
        <div class="px-6 space-y-4">
            <nav class="flex items-center gap-1.5 text-sm pt-4">
                ...
                <span class="font-semibold text-gray-700">Manage Asset: UPS</span>
            </nav>

            <div class="flex items-center justify-between flex-wrap gap-3">
                <div>
                    <p class="text-[10px] text-gray-400 uppercase tracking-wider font-bold">MANAGE ASSET</p>
                    <h1 class="text-xl font-bold text-pln-800 tracking-wide">Daftar Asset UPS (Uninterruptible Power Supply)</h1>
                </div>
                <div class="flex items-center gap-2">
                    <a href="{{ route('manage-ups.import.form') }}" class="inline-flex items-center gap-2 border border-gray-300 text-gray-700 px-4 py-2 rounded-md text-sm font-medium hover:bg-gray-50">
                    ...
                    <a href="{{ route('manage-ups.create') }}" class="inline-flex items-center gap-2 bg-[#004A54] text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-[#00363d]">
        ...
        <div class="mx-6 mt-4 bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
            <form method="GET" action="{{ route('manage-ups') }}" class="flex flex-row flex-wrap items-center gap-4">
                <div class="flex-1 min-w-[250px]">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari ID, merk, model, SN, IP..." class="w-full rounded-lg border border-gray-300 py-2.5 px-4 text-sm focus:border-[#004A54] focus:outline-none">
                </div>
                <div>
                ...
                <div>
                    <a href="{{ route('manage-ups') }}" class="inline-block border-2 border-[#004A54] text-[#004A54] px-5 py-2 rounded-lg text-sm font-bold hover:bg-cyan-50">Reset</a>
                </div>
            </form>
        </div>
        ...
        <div class="mx-6 mt-4 bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-10">
                ...
                <table class="w-full text-left border-collapse whitespace-nowrap">
                    <thead>
                        <tr class="bg-gray-50 border-b border-gray-200 text-xs font-bold uppercase tracking-wider text-gray-500">
                            ...
                            <th class="px-5 py-4">Tipe Kimia / Baterai</th>
                            <th class="px-5 py-4">Kapasitas Total</th>
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm">
                        @forelse(($upsList ?? []) as $ups)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                <td class="px-5 py-4">
                                    <div class="font-semibold text-[#004A54]">{{ $ups->id_aset ?? '-' }}</div>
                                    <div class="text-xs text-gray-500 mt-0.5">{{ $ups->merk ?? '-' }} {{ $ups->model ?? '' }}</div>
                                </td>
                                <td class="px-5 py-4 text-xs text-gray-700">
                                    <div class="font-semibold">{{ $ups->tipe_kimia ?: '-' }}</div>
                                    <div class="text-[10px] text-gray-400 mt-0.5">Jumlah: {{ $ups->jumlah_baterai ?? '-' }} unit</div>
                                </td>
                                <td class="px-5 py-4 text-xs text-gray-700">
                                    <div class="font-medium">{{ $ups->kapasitas_total ? $ups->kapasitas_total . ' kWh' : '-' }}</div>
                                </td>
                                <td class="px-5 py-4 text-xs font-mono text-gray-600">{{ $ups->ip_address ?? '-' }}</td>
                                <td class="px-5 py-4 text-xs text-gray-700">
                                    <div class="font-medium">{{ $ups->lokasi_aset_saat_ini ?? '-' }}</div>
                                    <div class="text-[10px] text-gray-400 mt-0.5">{{ $ups->keterangan_lokasi_aset ?? '-' }}</div>
                                </td>
                                <td class="px-5 py-4">
                                    <div class="flex items-center gap-1.5">
                                        <span class="inline-flex px-2 py-0.5 rounded-full text-[11px] font-medium {{ ($ups->status_kondisi ?? 'baik') == 'baik' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700' }}">
                                            {{ ucfirst($ups->status_kondisi ?? 'baik') }}
                                        </span>
                                        <span class="inline-flex px-2 py-0.5 rounded-full text-[11px] font-medium {{ ($ups->status_operasional ?? 'aktif') == 'aktif' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600' }}">
                                            {{ ucfirst($ups->status_operasional ?? 'aktif') }}
                                        </span>
                                    </div>
                                </td>
                                <td class="px-5 py-4 text-right sticky right-0 bg-white shadow-l">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('manage-ups.edit', $ups->id) }}" title="Edit" class="w-9 h-9 flex items-center justify-center rounded-md border border-gray-200 text-gray-500 hover:text-[#004A54] hover:border-[#004A54]">
                                        ...
                                        <form action="{{ route('manage-ups.destroy', $ups->id) }}" method="POST" onsubmit="return confirm('Hapus UPS {{ $ups->id_aset }}?')">
                                            @csrf @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-5 py-16 text-center text-gray-400">Belum ada data UPS.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            @if(isset($upsList) && method_exists($upsList, 'total'))
                <div class="px-5 py-4 border-t border-gray-100 text-sm text-gray-500 flex items-center justify-between">
                    <div>Menampilkan {{ $upsList->firstItem() ?? 0 }} - {{ $upsList->lastItem() ?? 0 }} dari {{ $upsList->total() }} data</div>
                    <div>{{ $upsList->onEachSide(1)->links() }}</div>
                </div>
            @endif
        </div>
    </div>
