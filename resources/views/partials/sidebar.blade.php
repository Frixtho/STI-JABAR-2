{{-- ===================== SIDEBAR ===================== --}}
<aside class="hidden lg:flex lg:flex-col lg:w-60 bg-pln-800 text-white shrink-0 overflow-y-auto min-h-screen border-r border-gray-200 dark:border-gray-800">
    ...
    <nav class="flex-1 px-4 space-y-1 mt-2 mb-6">
        ...
        @php
            // Cek status aktif menu Manage Asset (kecuali menu history)
            $assetActive = request()->is('manage-asset*') && !request()->routeIs('manage-asset.history*');
            
            // Cek status aktif menu Admin
            $adminActive = request()->routeIs('manage-user*') || request()->routeIs('manage-unit*') || request()->routeIs('manage-category*');
        @endphp
        ...
        <div>
            ...
            {{-- WADAH SUBMENU (Difilter Berdasarkan Global Aktif & Override Unit) --}}
            <div class="{{ $assetActive ? '' : 'hidden' }} ml-7 mt-1 border-l border-white/10 pl-3 space-y-1">
                @php
                    $userUnit = auth()->user()->department ?? '';

                    // Ambil kategori yang aktif secara global, lalu filter apakah ada pengecualian untuk unit ini
                    $dynamicCategories = \App\Models\AssetCategory::where('is_active', true)->orderBy('name')->get()->filter(function ($cat) use ($userUnit) {
                        $setting = \DB::table('category_unit_settings')
                            ->where('asset_category_id', $cat->id)
                            ->where('unit_name', $userUnit)
                            ->first();
                ...
                @foreach($dynamicCategories as $cat)
                    @php
                        // LOGIKA HYBRID: Cek apakah kategori ini adalah Tower (Statis) atau Generik
                        $routeUrl = $cat->slug === 'tower' ? route('manage-asset.tower.index') : route('manage-asset.generic.index', $cat->slug);
                        $isActive = $cat->slug === 'tower' ? (request()->routeIs('manage-asset.tower.*') || request()->is('manage-asset/tower*')) : request()->is('manage-asset/'.$cat->slug.'*');
                    @endphp
                    
                    <a href="{{ $routeUrl }}" 
                       class="relative block px-2 py-2 text-sm {{ $isActive ? 'text-white font-medium before:absolute before:-left-[13px] before:top-1/2 before:-translate-y-1/2 before:w-1 before:h-5 before:bg-accent-400 before:rounded-r' : 'text-pln-100 hover:text-white transition-colors' }}">
                        {{ $cat->name }}
                    </a>
                @endforeach
            </div>
        ...
        {{-- ===================== MENU ADMIN (HANYA UNTUK STI) ===================== --}}
        @if(auth()->check() && auth()->user()->role === 'STI')
        <div>
            <button type="button" 
                onclick="this.nextElementSibling.classList.toggle('hidden'); this.querySelector('.chevron-icon').classList.toggle('rotate-180');"
            ...
            <div class="{{ $adminActive ? '' : 'hidden' }} ml-7 mt-1 border-l border-white/10 pl-3 space-y-1">
                ...
                <a href="{{ route('manage-category') }}" 
                   class="relative block px-2 py-2 text-sm {{ request()->routeIs('manage-category*') ? 'text-white font-medium before:absolute before:-left-[13px] before:top-1/2 before:-translate-y-1/2 before:w-1 before:h-5 before:bg-accent-400 before:rounded-r' : 'text-pln-100 hover:text-white' }}">
        ...
        <div class="flex items-center gap-3 px-2 py-2 rounded-md bg-white/5">
            ...
            <div class="leading-tight min-w-0">
                ...
                <span class="inline-flex items-center mt-1 text-[9px] font-bold uppercase tracking-wide px-1.5 py-0.5 rounded
                    {{ strcasecmp(auth()->user()->role ?? '', 'STI') === 0 ? 'bg-accent-400 text-pln-800' : 'bg-white/10 text-pln-100' }}">
                    {{ auth()->user()->role ?? 'User' }}
                </span>
            </div>
        </div>
