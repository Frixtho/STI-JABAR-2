@extends('layouts.app', ['title' => (($asset ?? false) ? 'Edit Server Fisik' : 'Tambah Server Fisik') . ' — PLN Financial'])

@section('content')
<main class="p-6 lg:p-10 space-y-5 w-full">
    <nav class="flex items-center gap-1.5 text-sm">
        <a href="{{ route('manage-server-fisik') }}" class="text-gray-400 hover:text-[#004A54]">Server Fisik</a>
    ...
    <div>
        <h1 class="text-2xl font-bold text-pln-800">{{ ($asset ?? false) ? 'Edit Aset Server Fisik' : 'Tambah Aset Server Fisik Baru' }}</h1>
    </div>
    ...
    <form method="POST" action="{{ ($asset ?? false) ? route('manage-server-fisik.update', $asset->id) : route('manage-server-fisik.store') }}" class="bg-white rounded-xl border border-gray-200 shadow-sm">
        @csrf
        ...
        <div class="p-8 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mt-4">
                <div>
                    ...
                    <input type="text" name="keterangan_status_kepemilikan" value="{{ old('keterangan_status_kepemilikan', $asset->keterangan_status_kepemilikan ?? '') }}" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">
                </div>
                <div>
            ...
            {{-- ATRIBUT SPESIFIK HOST --}}
            <h3 class="text-xs font-bold uppercase tracking-wider text-[#004A54] mt-8 mb-4 pb-2 border-b">Atribut Spesifik Server (Host)</h3>
            
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div>
                    <label class="text-xs font-bold text-gray-700">Hostname Host</label>
                    <input type="text" name="hostname" value="{{ old('hostname', $asset->hostname ?? '') }}" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-700">Merk <span class="text-red-500">*</span></label>
                    <input type="text" name="merk" value="{{ old('merk', $asset->merk ?? '') }}" required class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-700">Model <span class="text-red-500">*</span></label>
                    <input type="text" name="model" value="{{ old('model', $asset->model ?? '') }}" required class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mt-4">
                <div>
                    ...
                    <input type="text" name="cpu" value="{{ old('cpu', $asset->cpu ?? '') }}" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-700">RAM</label>
                    <input type="text" name="ram" value="{{ old('ram', $asset->ram ?? '') }}" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-700">Disk Array</label>
                    <input type="text" name="disk_array" value="{{ old('disk_array', $asset->disk_array ?? '') }}" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-700">Kapasitas Storage Host</label>
                    <input type="text" name="kapasitas_total_storage" value="{{ old('kapasitas_total_storage', $asset->kapasitas_total_storage ?? '') }}" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mt-4">
                ...
                <div>
                    ...
                    <input type="text" name="ip_address" value="{{ old('ip_address', $asset->ip_address ?? '') }}" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-700">Sistem Operasi</label>
                    <input type="text" name="sistem_operasi" value="{{ old('sistem_operasi', $asset->sistem_operasi ?? '') }}" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">
                </div>
                <div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mt-4">
                <div>
                    <label class="text-xs font-bold text-gray-700">Rack / Posisi</label>
            ...
            {{-- ATRIBUT SERVER VIRTUAL --}}
            <h3 class="text-xs font-bold uppercase tracking-wider text-[#004A54] mt-8 mb-4 pb-2 border-b">Atribut Server Virtual (Guest)</h3>
            
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <div>
                    <label class="text-xs font-bold text-gray-700">Hostname Virtual</label>
                    <textarea name="hostname_virtual" rows="2" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">{{ old('hostname_virtual', $asset->hostname_virtual ?? '') }}</textarea>
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-700">Jumlah vCPU</label>
                    <textarea name="jumlah_vcpu" rows="2" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">{{ old('jumlah_vcpu', $asset->jumlah_vcpu ?? '') }}</textarea>
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-700">Kapasitas vRAM</label>
                    <textarea name="kapasitas_vram" rows="2" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">{{ old('kapasitas_vram', $asset->kapasitas_vram ?? '') }}</textarea>
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-700">Total Storage Virtual</label>
                    <textarea name="kapasitas_storage_virtual" rows="2" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">{{ old('kapasitas_storage_virtual', $asset->kapasitas_storage_virtual ?? '') }}</textarea>
                </div>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mt-4">
                <div>
                    <label class="text-xs font-bold text-gray-700">MAC Address Virtual</label>
                    <textarea name="mac_address_virtual" rows="2" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">{{ old('mac_address_virtual', $asset->mac_address_virtual ?? '') }}</textarea>
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-700">IP Address Virtual</label>
                    <textarea name="ip_address_virtual" rows="2" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">{{ old('ip_address_virtual', $asset->ip_address_virtual ?? '') }}</textarea>
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-700">Sistem Operasi Virtual</label>
                    <textarea name="sistem_operasi_virtual" rows="2" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">{{ old('sistem_operasi_virtual', $asset->sistem_operasi_virtual ?? '') }}</textarea>
                </div>
                <div>
                    <label class="text-xs font-bold text-gray-700">Aplikasi Virtual</label>
                    <textarea name="aplikasi_virtual" rows="2" class="mt-1.5 w-full rounded-md border border-gray-300 py-2.5 px-3 text-sm">{{ old('aplikasi_virtual', $asset->aplikasi_virtual ?? '') }}</textarea>
                </div>
            </div>
        ...
        <div class="flex justify-end gap-3 px-8 py-5 border-t bg-gray-50 rounded-b-xl">
            <a href="{{ route('manage-server-fisik') }}" class="border border-gray-300 text-gray-700 px-6 py-2.5 rounded-md text-sm hover:bg-gray-100">Batal</a>
            <button type="submit" class="bg-[#004A54] text-white px-6 py-2.5 rounded-md text-sm font-medium hover:bg-[#00363d]">
                {{ ($asset ?? false) ? 'Simpan Perubahan' : 'Simpan Server Fisik' }}
            </button>
        </div>
    </form>
</main>
