@extends('layouts.app', ['title' => 'Manage Server Baremetal — PLN Financial'])
    ...
    <div class="flex-1 min-w-0">
        ...
        <div class="px-6 space-y-4">
            <nav class="flex items-center gap-1.5 text-sm pt-4">
                ...
                <span class="font-semibold text-gray-700">Manage Asset: Server Baremetal</span>
            </nav>

            <div class="flex items-center justify-between flex-wrap gap-3">
                <div>
                    <p class="text-[10px] text-gray-400 uppercase tracking-wider font-bold">MANAGE ASSET</p>
                    <h1 class="text-xl font-bold text-pln-800 tracking-wide">Daftar Asset Server Baremetal</h1>
                </div>
                <div class="flex items-center gap-2">
                    <a href="{{ route('manage-server-baremetal.import.form') }}" class="inline-flex items-center gap-2 border border-gray-300 text-gray-700 px-4 py-2 rounded-md text-sm font-medium hover:bg-gray-50">
                    ...
                    <a href="{{ route('manage-server-baremetal.create') }}" class="inline-flex items-center gap-2 bg-[#004A54] text-white px-4 py-2 rounded-md text-sm font-medium hover:bg-[#00363d]">
        ...
        <div class="mx-6 mt-4 bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
            <form method="GET" action="{{ route('manage-server-baremetal') }}" class="flex flex-row flex-wrap items-center gap-4">
                ...
                <div>
                    <a href="{{ route('manage-server-baremetal') }}" class="inline-block border-2 border-[#004A54] text-[#004A54] px-5 py-2 rounded-lg text-sm font-bold hover:bg-cyan-50">Reset</a>
                </div>
            </form>
        </div>
                    ...
                    <tbody class="divide-y divide-gray-100 text-sm">
                        @forelse(($servers ?? []) as $server)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                {{-- Kolom Gabungan: ID Aset dan Merk/Model --}}
                                <td class="px-5 py-4">
                                ...
                                <td class="px-5 py-4 font-semibold text-gray-700">{{ $server->hostname ?? '-' }}</td>
                                ...
                                <td class="px-5 py-4 text-xs text-gray-700">{{ $server->sistem_operasi ?? '-' }}</td>
                                <td class="px-5 py-4 text-xs text-gray-700">
                                ...
                                <td class="px-5 py-4 text-right sticky right-0 bg-white shadow-l">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ route('manage-server-baremetal.edit', $server->id) }}" title="Edit" class="w-9 h-9 flex items-center justify-center rounded-md border border-gray-200 text-gray-500 hover:text-[#004A54] hover:border-[#004A54]">
                                        ...
                                        <form action="{{ route('manage-server-baremetal.destroy', $server->id) }}" method="POST" onsubmit="return confirm('Hapus server {{ $server->id_aset }}?')">
                                            @csrf @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="7" class="px-5 py-16 text-center text-gray-400">Belum ada data Server Baremetal.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
